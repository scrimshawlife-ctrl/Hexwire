...
enum SigilRecognizer {
    ...
    static func classify(points: [CGPoint]) -> SigilType {
        ...
        var horizFlips = 0
        var vertFlips = 0
        var sharpCorners = 0      // heading-deltas > 95° (was 80° — too jitter-sensitive)
        ...
        // Now also requires sharpCorners <= 1 so a vertical zigzag/lightning
        // can't sneak in here on aspect alone.
        if aspect > 2.0 && netDY > height * 0.55 && sharpCorners <= 1 {
            return .iceLance
        }

        // ---- 2. Zigzag → Lightning ----
        // Significantly loosened. A "Z" or "N" shape (single hard flip + 2
        // sharp corners) now counts. ALSO accepts vertical zigzags — many
        // players draw the lightning bolt symbol top-down instead of L→R.
        // Discriminator vs triangle: zigzag is OPEN (closure > 0.4), triangle
        // closes back near its start.
        if sharpCorners >= 2 && closure > 0.40 && pathLen > bbDiag * 1.15 &&
           ((horizFlips >= 1 && abs(netDX) > width * 0.30) ||
            (vertFlips  >= 1 && abs(netDY) > height * 0.30)) {
            return .lightning
        }

        // ---- 3. Triangle → Manabolt (CHECK BEFORE CIRCLE) ----
        // Tightened closure (0.65 → 0.50) — a real triangle closes within
        // half its diagonal. Sharp-corner cap raised (5 → 8) because each
        // physical corner may span 1–2 heading samples after the jitter
        // filter, so 3 corners can register as 3–6 sharpCorners.
        if sharpCorners >= 2 && sharpCorners <= 8 && closure < 0.50 && pathLen > bbDiag * 1.2 {
            return .manabolt
        }

        // ---- 4. Circle → Ward (smooth closed/near-closed loop) ----
        // sharpCorners <= 1 is the key discriminator now that the threshold
        // is 95° — accidental finger jitter under that bar doesn't trip the
        // triangle branch above.
        let bboxCircumference = .pi * (width + height) / 2
        if closure < 0.30 && pathLen > bboxCircumference * 0.55
           && sharpCorners <= 1 && totalTurning > .pi * 1.4 {
            return .ward
        }

        // ---- 5. Soul Drain → an OPEN CURVED SWEEP ----
        // A horizontal C / arc / wavy S. Way kinder to phone-finger gestures
        // than the old "draw a full spiral" requirement.
        ...
        // If the stroke has at least one sharp corner and isn't a closed
        // loop, manabolt is the safest guess — closure-based triangle
        // detection is otherwise the strictest path.
        if sharpCorners >= 1 && closure > 0.15 && pathLen > bbDiag * 1.3 {
            return .manabolt
        }
...
struct MirrorlineScene: View {
    ...
    private func spawnSpirit() {
        ...
        let flicker: SigilType? = (stage == .staticEscalation && Bool.random())
            ? AstralEnemy.allCases.first(where: { $0 != kind })?.weakness
            : nil
    ...
    private func resolveSigil(_ sig: SigilType) {
        ...
        // Find the nearest non-banished spirit
        guard let target = nearestActiveSpirit() else {
        ...
        if sig == target.0.effectiveWeakness {
            ...
            // Wrong sigil — chip the target a little, self-chip Sable a little
            spirits[target.1].banished += 0.15   // visual ding, not banish
            applyDamageToPlayer(amount: 1)
