...
struct LaneRunnerMiniGame: View {
    ...
    /// 2026-05: 10 → 14 to make the dive run a bit longer.
    private let targetFragments = 14
    ...
    /// 2026-05: 0.0083 → 0.0072 so the longer 14-fragment run stays winnable
    /// (keeps trace pressure-per-fragment roughly constant).
    private let traceDrift: CGFloat = 0.0072
    ...
    struct Obstacle: Identifiable {
        ...
        /// Data shards only: when true the shard floats above the lane and can
        /// ONLY be grabbed mid-jump. Ground shards can only be grabbed while
    ...
    /// Jumps taken since last grounded. Allows a DOUBLE jump (max 2) but not
    /// a third — resets to 0 on landing, so it can't be spammed into infinite
    /// hover.
    @State private var jumpsUsed: Int = 0
    /// Hard ceiling on jump height so a double jump can't climb off-screen.
    private let maxJumpHeight: CGFloat = 150
                ...
                MiniGameBriefing(
                    ...
                    rules: [
                        ...
                        BriefingRule(
                            ...
                            color: Color(hex: "00FFCC"),
                            title: "GRAB DATA SHARDS",
                            detail: "Collect \(targetFragments) fragments before the trace fills or you take \(maxHits) hits. Cyan shards sit on the ground — grab them grounded. Amber '▲ JUMP' shards float — JUMP to snag those."
                        ),
                    ],
                    onStart: {
                        showBriefing = false
    ...
    private func drawObstacles(ctx: GraphicsContext, size: CGSize) {
        ...
        for ob in ordered {
            ...
            case .dataShard:
                let lane = ob.lanes.first ?? 1
                var pos = screenPos(lane: lane, z: ob.z, in: size)
                if ob.airborne {
                    // Float it up off the lane, and draw a dashed tether from the
                    // lane floor up to it so it unmistakably reads as ELEVATED
                    // (jump to reach) rather than a ground pickup you can run into.
                    let groundY = pos.y
                    // Lift it clearly off the lane (more than before) so it reads
                    // as airborne even far back, not a ground pickup.
                    pos.y -= (22 + 95 * p)
                    if p > 0.10 {
                        ctx.stroke(
                            Path { pa in
                                pa.move(to: CGPoint(x: pos.x, y: groundY))
                                pa.addLine(to: CGPoint(x: pos.x, y: pos.y + 10))
                            },
                            with: .color(Color(hex: "FFCC33").opacity(0.5)),
                            style: StrokeStyle(lineWidth: max(1, p * 1.5), dash: [3 * p + 1, 3 * p + 1])
                        )
                    }
                }
                drawDataShard(ctx: ctx, center: pos, scale: scale, airborne: ob.airborne)
            case .traceBeam:
    ...
    private func drawDataShard(ctx: GraphicsContext, center: CGPoint, scale: CGFloat, airborne: Bool = false) {
        // Diamond shard with halo + sparkle. Floating shards read amber with a
        // little "▲ JUMP" cue so the player knows to leap for them; ground
        // shards stay cyan.
        let core = airborne ? Color(hex: "FFCC33") : Color(hex: "00FFCC")
        let halfSize = 8 + 14 * scale
        let halo = CGRect(x: center.x - halfSize - 6,
                          y: center.y - halfSize - 6,
                          width: (halfSize + 6) * 2, height: (halfSize + 6) * 2)
        ctx.fill(Path(ellipseIn: halo), with: .color(core.opacity(0.18)))
        var path = Path()
        ...
        // Jump cue for floating shards — shown EARLY (from far back) and in
        // high-contrast white so it doesn't disappear into the amber shard.
        // This is what tells the player the "orange diamond" must be JUMPED for.
        if airborne && scale > 0.22 {
            ctx.draw(
                Text("▲ JUMP")
                    .font(.system(size: 9 * scale + 6, weight: .black, design: .monospaced))
                    .foregroundColor(.white),
                at: CGPoint(x: center.x, y: center.y - halfSize - 13)
            )
        }
    }
    ...
    private func tryJump() {
        guard !gameOver else { return }
        // DOUBLE jump: allow a second launch in mid-air, but never a third
        // before landing — `jumpsUsed` (reset to 0 on touchdown) caps it at 2,
        // so a swipe-per-frame can't hover indefinitely or climb off-screen.
        guard jumpsUsed < 2 else { return }
        jumpsUsed += 1
        ...
        // The second jump RE-SETS velocity (doesn't stack onto the upward
        // momentum) so the apex stays bounded — a slightly softer −360 reads
        // as an "air hop" rather than a rocket.
        playerJumpVel = (jumpsUsed == 1) ? -420 : -360
        chiptune?.sfxJump()
        HapticsManager.shared.buttonTap()
    }
    ...
    private func tick(dt: CGFloat) {
        ...
        if playerJumpVel != 0 || playerJumpY < 0 {
            ...
            // Clamp height so a double jump can't push the runner off-screen.
            if playerJumpY < -maxJumpHeight {
                playerJumpY = -maxJumpHeight
                if playerJumpVel < 0 { playerJumpVel = 0 }   // stop rising; gravity pulls back down
            }
            if playerJumpY >= 0 {
                playerJumpY = 0
                playerJumpVel = 0
                jumpsUsed = 0   // grounded — refresh the double jump
            }
        }
    ...
    private func resolveObstacle(at i: Int) {
        ...
        case .dataShard:
            // You can only grab a shard at your own elevation: floating shards
            // require a jump; ground shards require both feet down. A shard at
            // the wrong elevation simply whiffs past (not consumed, no penalty).
            let elevationMatches = obstacles[i].airborne ? isJumping : !isJumping
            if inLane && elevationMatches {
                dataFragments += 1
                chiptune?.sfxCollect()
    ...
    private func spawnObstacle() {
        ...
        if roll < 33 {
            ...
            kind = .dataShard
            lanes = [Int.random(in: 0..<laneCount)]
            // ~50% of shards float — the runner must JUMP to grab those, and
            // can only collect ground shards while grounded.
            airborne = Int.random(in: 0..<100) < 50
        } else if roll < 85 {
        ...
        obstacles.append(Obstacle(z: 0, lanes: lanes, kind: kind, airborne: airborne))
    }
