...
struct PacketRouterMiniGame: View {
    ...
    /// History: 0.018 → 0.012 → 0.016 → 0.012 → 0.009.
    private let traceDrift: CGFloat = 0.009
    /// ICE moves once every this many seconds.
    /// History: 1.10 → 1.30 → 1.00 → 1.25 → 1.5.
    private let iceTickSeconds: Double = 1.5
    /// Grace period at start of run before ICE moves at all.
    /// History: 1.5s → 2.5s → 3.0s.
    private let iceGraceSeconds: Double = 3.0
    ...
    private func tickICE() {
        ...
        for (i, iceId) in iceNodeIds.enumerated() {
            ...
            // Don't hop onto a shielded node (player's safe zones)
            let allowed = n.filter { !nodes[$0].isShielded || $0 == packetNodeId }
