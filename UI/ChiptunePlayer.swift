...
final class ChiptunePlayer {
    ...
    /// Session observers (interruption / route change) — an interruption
    /// stops the AVAudioEngine while `isStarted` stays true, so without a
    /// restart hook the mini-game goes silent while tickMelody keeps
    /// scheduling buffers onto a dead engine.
    ...
    init?() {
        ...
        // Derive from MusicManager.targetVolume so the chiptune mini-game
        // music sits at the same loudness as the mission soundtrack we just
        // ducked (0.722 ≈ the tuned 0.397 / the 0.55 default — retuning
        // targetVolume now scales the chiptune with it instead of silently
        // desyncing the two).
        let chipVolume = MusicManager.shared.targetVolume * 0.722
        leadPlayer.volume = chipVolume
        bassPlayer.volume = chipVolume
        drumPlayer.volume = chipVolume
        sfxPlayer.volume  = chipVolume

        do {
            // The audio session is configured ONCE by MusicManager (.playback,
            // no options). Re-setting the category here with .mixWithOthers —
            // on an active session, mid-mission — permanently flipped the
            // whole app's audio policy after the first mini-game (Spotify
            // playing over the game) and could glitch the paused music player.
        ...
        // Recover the engine after a phone call / Siri / headphones unplug —
        // the system stops AVAudioEngine and nothing restarts it otherwise.
    ...
    private func restartEngineIfNeeded() {
        ...
        do {
            ...
            // Engine unrecoverable — stop scheduling onto it.
            isStarted = false
        }
    }
