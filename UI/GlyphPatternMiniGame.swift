...
struct GlyphPatternMiniGame: View {
    ...
    /// One-shot latch for finishGame — every sibling game guards this;
    /// without it a BAIL double-tap (or BAIL racing the scheduled loss
    /// closure) delivered onComplete → resolveMatrixMiniGame twice.
    @State private var resultDelivered: Bool = false
    ...
    private func handleTap(at point: CGPoint, in size: CGSize) {
        ...
        if idx == expected {
            ...
            if mistakes >= maxMistakes {
                ...
                didWin = false
                chiptune?.sfxLose()
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.9) {   // snap back fast on a loss
                    finishGame(success: false)
                }
                return
            }
            // Close input IMMEDIATELY — leaving phase at .awaitingInput for
            // the replay window let the player keep tapping from the stale
            // index (finishing the round, skipping the penalty) and stack
            // strikes + overlapping playback chains that garbled the display.
            phase = .playback
    ...
    private func roundCleared() {
        ...
        if currentRound >= totalRounds {
            ...
            phase = .finished
            chiptune?.sfxWin()
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {   // linger on a win
