...
struct Weapon: Codable, Equatable, Identifiable, Hashable {
    ...
    /// Explicit stun/physical override. nil = infer from the weapon type
    /// (unarmed = stun). Lets a magical nuke ride the .unarmed delivery
    /// without becoming nonlethal — Sato's Bloodbolt was silently dealing
    /// stun, so the finale boss's opening hits couldn't kill. Optional with
    /// a default, so saved weapons without the key decode fine.
