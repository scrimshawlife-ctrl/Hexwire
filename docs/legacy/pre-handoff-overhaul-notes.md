# HexWire Game — Overhaul Handoff

**Purpose:** Drop-in context for a fresh Claude session (e.g., Sonnet) to continue the aggressive overhaul of the HexWire-themed turn-based RPG without re-discovery. Read this file top-to-bottom before touching code.

---

## 1. User goals (from Aaron, abgodbout@gmail.com)

> "make hexwire game as good as possible. improve this dramatically. want it to be a playable, slick looking cyberpunk turn based rpg."
...
| File | Role |
| --- | --- |
| `/Users/prabu/.openclaw/workspace-coding/HEXWIRE-SPEC.md` | Design spec. FTL meets HexWire. 4 archetypes, TN6 dice, mission JSON format. |
| `/Users/prabu/.openclaw/workspace-socials/HEXWIRE-LORE.md` | Lore/narrative content — Raze, Sable, Cipher, Lyra backstories, radio chatter, flavor text. |
| `/Users/prabu/.openclaw/workspace-coding/HexWire/` | The Xcode project. |
...
```
HexWire/
├── HexwireApp.swift        46KB  entry point, PhaseManager FSM, ContentView, CombatView, BattleSceneView wrapper
...
├── Sprites/
└── HexWire.xcodeproj/      generated by XcodeGen from project.yml
```
...
In `GameState.swift`, replace the current single `performSpell()` with `performSpell(spellId: String)`. Keep the existing no-arg `performSpell()` as a default that picks `"fireball"` for backwards compat with `HexwireApp.swift`'s `onSpell: { gameState.performSpell() }` callback.
...
- Ask Aaron to run `xcodegen generate` in `HexWire/` then build from Xcode.
...
```
1. Read this file (HEXWIRE-OVERHAUL-HANDOFF.md) top to bottom.
