...
final class MusicManager {
    ...
    private enum DeferredKind {
        case loop(filename: String, startOffset: TimeInterval)
    ...
    func playLoop(filename: String, startOffset: TimeInterval = 0) {
        let id = "loop:\(filename)"
        guard id != currentTrackId else { return }
        if isDucked {
            currentMissionId = nil
            currentTrackId = id
            deferredResumeKind = .loop(filename: filename, startOffset: startOffset)
            return
        }
        crossFadeOut { [weak self] in
            guard let self = self else { return }
        ...
        crossFadeOut { [weak self] in
            ...
            do {
                ...
                if !loop, let chain = self.bossChain {
                    let lead = max(0.4, chain.endTrimSeconds)
                    let triggerTime = max(0.5, p.duration - lead)
                    DispatchQueue.main.asyncAfter(deadline: .now() + triggerTime) { [weak self, weak p] in
                        guard let self = self,
                              let pp = p,
                              self.player === pp else { return }   // bail if chain was replaced
    ...
    private func advanceBossChain() {
        ...
        // already paused. unduck() will pick up cleanly.
    ...
    func unduck() {
        ...
        if let deferred = deferredResumeKind {
            ...
            switch deferred {
            case .loop(let filename, let offset):
                currentTrackId = nil   // force playLoop to act
                playLoop(filename: filename, startOffset: offset)
        ...
        fade(toVolume: targetVolume, duration: 0.6)
    }
    ...
    private func startLoopingTrack(filename: String, startOffset: TimeInterval = 0) {
        ...
        do {
            let p = try AVAudioPlayer(contentsOf: url)
            p.numberOfLoops = -1     // infinite loop — AVAudioPlayer wraps the file seamlessly enough for ambient beds
            p.volume = 0.0
            p.prepareToPlay()
    ...
    private func startTrack(missionId: String, slot: String, fadeIn: Bool, startOffset: TimeInterval = 0) {
        ...
        switch filename {
        case "m2_a": trackBoost = 1.10  // Berlin Shadows side A — +10%
    ...
    private func scheduleEndObserver(for player: AVAudioPlayer) {
        ...
        // the "weird quiet time" gap between tracks.
        let pollDuration = player.duration
        let total = max(1.0, pollDuration)
        DispatchQueue.main.asyncAfter(deadline: .now() + max(0.5, total - 1.0)) { [weak self] in
            guard let self = self else { return }
            // Bail if mission was stopped or another track was started in the meantime.
            guard self.currentMissionId != nil, self.player === player else { return }
    ...
    private func advanceToNextSlot() {
        ...
        // mini-game. unduck() will resume the paused track in place.
        guard !isDucked else { return }
    ...
    private func fade(toVolume target: Float, duration: TimeInterval, completion: (() -> Void)? = nil) {
        ...
        fadeTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 30.0, repeats: true) { [weak self] timer in
            ...
            p.volume = startVolume + Float(t) * (target - startVolume)
            if t >= 1.0 {
                timer.invalidate()
                self.fadeTimer = nil
                completion?()
