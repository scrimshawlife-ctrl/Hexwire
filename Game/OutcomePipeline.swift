...
struct OutcomePipeline {
    ...
    static func execute(
        ...
        if won, let missionId = gameState.currentMissionDisplayId {
            ...
            // Participation XP — EVERY runner who deployed earns mission-
            // completion XP, not just whoever landed the killing blows. Kill XP
            // alone meant support runners (especially the decker, who hacks far
            // more than he kills) never levelled. Folds into each runner's
            // mission-gain summary so the debrief scorecard reflects it.
            let participationXP = 18 + gameState.missionEnemiesDefeated * 3
            for runner in gameState.playerTeam {
                let leveled = runner.gainXP(participationXP)
                if leveled {
                    gameState.addLog("🎖️ LEVEL UP! \(runner.name) → Level \(runner.level)!")
                    NotificationCenter.default.post(name: .characterLevelUp, object: nil,
                                                    userInfo: ["characterId": runner.id.uuidString])
                }
            }
            gameState.addLog("◆ +\(participationXP) mission XP to all runners")

            // Persist roster progression (levels/XP/stats earned this mission)
            // so it carries into the next mission AND across NG+ replays.
            RosterStore.shared.save(gameState.playerTeam)

            // Completing the finale advances New Game+ — the next run keeps the
            // roster but scales enemy difficulty up a tier.
            if missionId == "Mission006" {
                NGPlusStore.shared.advance()
                // New run-through: every mission can earn its payout again.
                MissionStatsStore.shared.resetPaidThisRun()
                TutorialCoach.shared.enqueue(.newGamePlus)   // explain NG+ on the debrief
                gameState.addLog("🏆 CAMPAIGN COMPLETE — New Game+\(NGPlusStore.shared.tier) unlocked. Your runners carry forward.")
            }
        }
    ...
    static func applyFactionAttention(gameState: GameState, traceTier: Int) {
        ...
        MissionStatsStore.shared.saveFactionAttention(gameState.factionAttention)
    }
